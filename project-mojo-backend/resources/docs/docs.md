...
The `home-routes` handler in the `project-mojo-backend.routes.home` namespace
...
The `home-page` function will in turn call the `project-mojo-backend.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `project-mojo-backend.handler` namespace:
...
Request middleware functions are located under the `project_mojo_backend.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `project-mojo-backend.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `project-mojo-backend.db.core` in some other namespace.
